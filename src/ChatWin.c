...
static char firstuser[40];
static char seconduser[40];
int row, col, endrow, endcol;
char move[5];
int isNewGame = 0;

int ChatWin(int argc, char *argv[], char *user1, char *user2, int showboard, char *initialmessage) {             //user1 is one who started it, user two is one who is going to receive message
    ...
    //GtkWidget *TextLabel;
    strcpy(firstuser, user1);
    strcpy(seconduser, user2);
    gtk_init (&argc, &argv);
    /*create a new window */
    ...
    //gtk_text_buffer_apply_tag (buffer, justifyR, &start, &end);
    ...
    if (initialmessage) {
        gtk_text_buffer_set_text(buffer, (const gchar*) initialmessage, (gint) 256);
    }
...
*/
    ...
    g_signal_connect (MoveButton, "clicked", G_CALLBACK (movemade), NULL);
...
*/        
        ...
        strcat(Buffer," ");
        strcat(Buffer, (char *) gtk_entry_get_text(GTK_ENTRY(MessageEntry)));
        strcat(Buffer," ");
        strcat(Buffer, seconduser);

        gtk_text_buffer_set_text(buffer, gtk_entry_get_text(GTK_ENTRY(MessageEntry)), -1);
        gtk_text_view_set_buffer(GTK_TEXT_VIEW(ChatLog), buffer);
        //strcat(Buffer,Text);
...
gboolean fight (GtkWidget *widget,
         GdkEvent  *event,
         gpointer   data) 
{
    char *Response;
...
gboolean movemade (GtkWidget *widget,
         GdkEvent  *event,
         gpointer   data) 
{
    ...
    if(gtk_entry_get_text_length(GTK_ENTRY(StartEntry)) == 2 && gtk_entry_get_text_length(GTK_ENTRY(EndEntry)) == 2){
        ...
        if(isNewGame){
            fromPos = (char *) gtk_entry_get_text(GTK_ENTRY(StartEntry));
            toPos = (char *) gtk_entry_get_text(GTK_ENTRY(EndEntry));
